...
const endCallScript = `if redis.call('EXISTS', KEYS[1]) == 0 then return 2 end; local caller = redis.call('HGET', KEYS[1], 'caller_id'); local recipient = redis.call('HGET', KEYS[1], 'recipient_id'); local callerDevice = redis.call('HGET', KEYS[1], 'caller_device_id'); local acceptedDevice = redis.call('HGET', KEYS[1], 'accepted_device_id'); if (ARGV[1] ~= caller or ARGV[2] ~= callerDevice) and (ARGV[1] ~= recipient or ARGV[2] ~= acceptedDevice) then return 3 end; if redis.call('HGET', KEYS[1], 'status') == 'ended' then return 2 end; local id = string.sub(KEYS[1], 11); redis.call('HSET', KEYS[1], 'status', 'ended'); redis.call('EXPIRE', KEYS[1], ARGV[6]); if redis.call('GET', 'zwei:call:user:' .. caller) == id then redis.call('DEL', 'zwei:call:user:' .. caller) end; if redis.call('GET', 'zwei:call:user:' .. recipient) == id then redis.call('DEL', 'zwei:call:user:' .. recipient) end; if redis.call('GET', 'zwei:call:device:' .. caller .. ':' .. callerDevice) == id then redis.call('DEL', 'zwei:call:device:' .. caller .. ':' .. callerDevice) end; if redis.call('GET', 'zwei:call:device:' .. recipient .. ':' .. acceptedDevice) == id then redis.call('DEL', 'zwei:call:device:' .. recipient .. ':' .. acceptedDevice) end; redis.call('ZREM', KEYS[3], id); return 1`
const expireCallsScript = `local ids = redis.call('ZRANGEBYSCORE', KEYS[1], '-inf', ARGV[1]); for _, id in ipairs(ids) do local key = 'zwei:call:' .. id; if redis.call('EXISTS', key) == 1 then local caller = redis.call('HGET', key, 'caller_id'); local recipient = redis.call('HGET', key, 'recipient_id'); local callerDevice = redis.call('HGET', key, 'caller_device_id'); local acceptedDevice = redis.call('HGET', key, 'accepted_device_id'); redis.call('HSET', key, 'status', 'ended'); redis.call('EXPIRE', key, 60); if redis.call('GET', 'zwei:call:user:' .. caller) == id then redis.call('DEL', 'zwei:call:user:' .. caller) end; if redis.call('GET', 'zwei:call:user:' .. recipient) == id then redis.call('DEL', 'zwei:call:user:' .. recipient) end; if redis.call('GET', 'zwei:call:device:' .. caller .. ':' .. callerDevice) == id then redis.call('DEL', 'zwei:call:device:' .. caller .. ':' .. callerDevice) end; if redis.call('GET', 'zwei:call:device:' .. recipient .. ':' .. acceptedDevice) == id then redis.call('DEL', 'zwei:call:device:' .. recipient .. ':' .. acceptedDevice) end; end; redis.call('ZREM', KEYS[1], id); end; return ids`
